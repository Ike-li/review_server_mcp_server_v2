...
def test_ip_few():
    ...
    s = score_ip_distribution(records)
    assert s.raw_score == 0
...
def test_payment_some():
    ...
    s = score_payment(records)
    assert s.raw_score == 100  # 5 个付费即满分


def test_device_few():
    records = [_make_record(uid=f"u{i}", device_id="dev-1") for i in range(5)]
    s = score_device_distribution(records)
    assert s.raw_score == 0
...
def test_all_dimensions_empty_records():
    """所有维度评分器应安全处理空列表。"""
    server = _make_server()
    assert score_formal_crosscheck([], []).raw_score == 0
    assert score_player_count([], threshold=20).raw_score == 0
    assert score_ip_distribution([]).raw_score == 0
    assert score_payment([]).raw_score == 0
    assert score_device_distribution([]).raw_score == 0
    assert score_channel_version([], server).raw_score == 0
    assert score_register_time([], DT).raw_score == 0
    assert score_geo_distribution([]).raw_score == 0
